...
class DBStorage(StorageInterface):
    ...
    async def get_metadata(self, model_id: str) -> StorageMetadata | None:
        ...
        if not await self.dataset_exists(input_dataset):
            return None

        def _schema_from_columns(names: list[str]) -> Schema:
            items = {
                name: SchemaItem(DataType.DOUBLE, name, i)
        ...
        try:
            ...
            input_schema = _schema_from_columns(input_names)
            n_rows = await self.dataset_rows(input_dataset)
        except Exception as e:  # Intentional: per-dataset errors should not crash
            logger.warning("Error reading input metadata for %s: %s", model_id, e)
            return None

        try:
            output_names = await self.get_original_column_names(output_dataset)
            output_schema = _schema_from_columns(output_names)
        except Exception:  # Intentional: output may not exist yet
    ...
    async def get_partial_payload(
        self, payload_id: str, *, is_input: bool, is_modelmesh: bool = False
